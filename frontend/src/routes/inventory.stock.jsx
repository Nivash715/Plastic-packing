// inventory.stock.jsx — Stock Management (converted from TSX to JSX)
...
function Page() {
  ...
  const [addForm, setAddForm] = useState({ name: "", cat: "Cardboard", qty: "", unit: "pcs", min: "" });
  ...
  const saveEdit = async (id) => {
    ...
    try {
      await fetch(`http://localhost:8000/api/stock/${id}`, {
        method: "PUT",
  ...
  const deleteItem = async (id) => {
    ...
    try { await fetch(`http://localhost:8000/api/stock/${id}`, { method: "DELETE" }); } catch {}
    setItems((prev) => prev.filter((i) => i.id !== id));
  };

  const handleAdd = async () => {
    if (!addForm.name || !addForm.qty) return;
    const newItem = { ...addForm, qty: parseFloat(addForm.qty), min: parseFloat(addForm.min) || 0 };
    try {
      ...
      if (res.ok) { const created = await res.json(); setItems((p) => [...p, created]); }
    } catch {
      setItems((p) => [...p, { ...newItem, id: Date.now().toString() }]);
    }
    setAddForm({ name: "", cat: "Cardboard", qty: "", unit: "pcs", min: "" });
    setShowAdd(false);
  };
  ...
  return (
    <DashboardLayout title="Stock" subtitle="Manage packaging materials and inventory levels." lowStockItems={items.filter((i) => i.qty < i.min)}>
      {/* KPIs */}
      ...
      {showAdd && (
        <Section title="Add Stock Item">
          ...
          <div className="mt-4 flex gap-2">
            ...
            <Btn variant="ghost" onClick={() => setShowAdd(false)}><X className="h-4 w-4" /> Cancel</Btn>
          </div>
        </Section>
      )}

      {showAdd && <div className="h-5" />}
                    ...
                    {editingId === item.id ? (
                      <div className="flex items-center gap-1">
                        <input
                          ...
                          autoFocus
                        />
                        <button onClick={() => saveEdit(item.id)} className="text-primary hover:text-primary/80 p-1">
